...
wp_enqueue_script('stm-form-listing', get_template_directory_uri() . '/assets/js/ulisting/stm-form-listing.js', array('vue'), ULISTING_VERSION, true);
...
if(isset($attributeIds['category'])) {
	$data['attributes']['category'] = array(
		...
		'name'     => 'category',
		'type'     => 'category',
		'options'  => StmListingCategory::getListDataArray(),
		'value'    => array(),
	);
}
...
<div id="stm-listing-form-listing">
	<div class="step" v-if="step == 'form'">
		<div class="select-plan-form">
			...
			<div class="stm-row">
				<div v-for="plan in user_plans" class="stm-col-4" v-if="plan.listing_limit || (plan.payment_type == 'one_time' && plan.id == listing_plan_select)">
					<div class="card">
						<div class="card-body">
							<span v-if="plan.id == listing_plan_select" class="badge badge-success">Selected</span>
							<h5 class="card-title">{{plan.name}}</h5>
							<p v-if="plan.payment_type == 'subscription' || plan.id == 'free'" class="card-text"> {{plan.listing_limit}} / {{plan.use_listing_limit}}</p>
							<p v-if="plan.payment_type == 'one_time'" class="card-text"> Limit : {{plan.listing_limit}}</p>
							<v-timer
							...
							<button v-if="!listing_plan_one_time" @click="select_limit_plan(plan)" class="btn btn-primary">select</button>
						</div>
					</div>
				</div>
			</div>
				...
				<div v-for="plan in user_plans" class="stm-col-3" v-if="plan.feature_limit">
					<div class="card">
						<div class="card-body">
							...
							<h5 class="card-title">{{plan.name}}</h5>
							<p class="card-text"> {{plan.feature_limit}} / {{plan.use_feature_limit}} </p>
