...
class MultiplePlan
{
	...
	public function __construct() {
		...
		$subscriptionOption = get_option('subscriptio_options', '');

		if($subscriptionOption && $subscriptionOption['subscriptio_multiproduct_subscription']) {
	...
	public function buildPlansMeta() {
		...
		if(self::$currentUser && class_exists('Subscriptio_User')) {
			$user_subscriptions = Subscriptio_User::find_subscriptions( true, self::$currentUser );
			if($user_subscriptions) {
				foreach ( $user_subscriptions as $user_subscription) {
					...
					self::$plansMeta['plans'][] = array(
						'plan_id' => $user_subscription->id,
						'label' => ( !empty( $user_subscription->products_multiple ) ) ? $user_subscription->products_multiple[0]['product_name'] : $user_subscription->product_name,
						'total_quota' => $post_limit,
						'used_quota' => self::getUsedQuotaByPlanId($user_subscription->id),
