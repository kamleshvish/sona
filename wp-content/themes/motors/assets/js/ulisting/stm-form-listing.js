if (typeof stm_listing_form_listing != "undefined") {
    ...
    new Vue({
        ...
        methods: {
            ...
            mceSetter: function () {
                let vm = this;
                let iframes = document.querySelectorAll('.mce-tinymce iframe');
            ...
            buildData: function () {
                ...
                if (this.feature_image) {
                    vm.formData.append('feature_image[attr]', this.feature_image.attr);
                    vm.formData.append('feature_image[index]', this.feature_image.index);
                }
                ...
                for (key in this.attributes) {
                    switch (vm.attributes[key].type) {
                        ...
                        case 'gallery':
                            var name = this.attributes[key].name;
                            for (i in this.attributes[key].value) {
                                if (this.attributes[key].value[i].id) {
                                    vm.formData.append('options[' + name + '][' + this.attributes[key].value[i].id + ']', this.attributes[key].value[i].value);
                                } else {
                                    vm.formData.append('options[' + name + '][' + i + ']', null);
                                    vm.formData.append(name + '[]', this.attributes[key].value[i].file);
                                }
                            }
                            break;
                        case 'accordion':
            ...
            send: function () {
                ...
                this.$http.post(currentAjaxUrl + '?action=stm_listing_ajax', vm.formData).then(function (response) {
                    ...
                    if (vm.status == 'success') {
                        ...
                        $('.add-listing-navigate').find('.step').removeClass('active');
                        $('.add-listing-navigate').find('.step:last-child').addClass('active');
